...
public class Slave {
    ...
    public void start() {
        ...
        try (ServerSocket serverSocket = new ServerSocket(listenPort)) {
            System.out.println("SlaveServer écoute sur le port " + listenPort);
            while (true) {
    ...
    public static void main(String[] args) {
        if (args.length < 5) {
            System.err.println("Usage: java slaves.SlaveServer <slaveId> <masterHost> <masterPort> <listenPort> [storageDir]");
            System.exit(1);
        }
        try {
